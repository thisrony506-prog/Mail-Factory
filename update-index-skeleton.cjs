...
const skeleton = `
      <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100vh;background-color:#0b0f19;">
        <div style="width:60px;height:60px;border-radius:16px;background:linear-gradient(135deg, #4f46e5, #7e22ce);display:flex;align-items:center;justify-content:center;box-shadow:0 10px 25px rgba(79,70,229,0.3);position:relative;animation:mf-pulse 2s cubic-bezier(0.4, 0, 0.6, 1) infinite;">
...
code = code.replace('<div id="root"></div>', `<div id="root">${skeleton}</div>`);
